...
> 
> 💡 **vGPU (Hami) 資源限制寫入 answers 欄位**：
> - 若 `gpu_memory_limit_mib > 0`，代表目前環境啟用了 vGPU 虛擬化切割（Hami 雙調度器）。為了避免第一個部署的服務獨佔整張顯示卡的所有 VRAM，導致後續服務因 `CardInsufficientMemory` 而卡在 Pending 狀態，**必須** 在 `answers` 中加入 `"values.resources"` 欄位以限制該 Pod 的顯存和核心使用量：
>   ```json
>   "values.resources": {
>     "requests": {
